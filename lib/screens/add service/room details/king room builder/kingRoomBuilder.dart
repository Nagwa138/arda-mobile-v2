...
class KingRoomBuilder extends StatelessWidget {
  const KingRoomBuilder({super.key});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 20.h),
        context.read<AddServiceCubit>().kingRoomImage == null
            ? Row(
                children: [
                  Expanded(
                    child: GestureDetector(
                      onTap: () {
                        context.read<AddServiceCubit>().addKingRoomImage(isPicked: true);
                      },
                      child: DashedRect(
                        color: accentColor,
                        strokeWidth: 1,
                  ...
                  Expanded(
                    child: GestureDetector(
                      onTap: () {
                        context.read<AddServiceCubit>().addKingRoomImage(isPicked: false);
                      },
                      child: DashedRect(
                        color: accentColor,
                        strokeWidth: 1,
            ...
            : Column(
                ...
                children: [
                  Image.file(
                    File(context.read<AddServiceCubit>().kingRoomImage!.path),
                    // height: 100.h,
                    fit: BoxFit.cover,
                  ),
                  ...
                  GestureDetector(
                    onTap: () {
                      context.read<AddServiceCubit>().removeKingRoomImage();
                    },
                    child: Text(
        ...
        TextFormField(
          controller: context.read<AddServiceCubit>().kingRoomNightController,
          validator: (value) {
            if (value!.isEmpty) {
        ...
        SizedBox(height: 15.h),
        TextFormField(
          controller: context.read<AddServiceCubit>().kingRoomGuestController,
          validator: (value) {
            if (value!.isEmpty) {
              return 'errorMessages.empty'.tr();
            }
        ...
        CheckboxListTile(
          ...
          value: context.read<AddServiceCubit>().agreeKingSize,
          onChanged: (newValue) {
            context.read<AddServiceCubit>().changeAgreeKingSize(newValue);
          },
        ),
      ],
    );
